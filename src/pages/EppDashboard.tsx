...
const categoryProtectionMap: Record<string, string> = {
  'Protección para cabeza': 'Impactos, objetos en caída, electricidad y ambientes industriales exigentes.',
  'Protección auditiva': 'Reducción de exposición a ruido continuo o intermitente en planta.',
  'Protección ocular': 'Proyección de partículas, radiación lumínica y salpicaduras químicas.',
  'Protección respiratoria': 'Filtrado de vapores, gases y material particulado en suspensión.',
  'Protección para manos': 'Cortes, abrasión, químicos y riesgos eléctricos en extremidades.',
  'Protección para pies': 'Impacto, compresión, perforaciones y resbalones en superficie.',
  'Protección corporal': 'Agentes químicos, salpicaduras, fuego y condiciones climáticas.',
  'Arneses y líneas de vida': 'Trabajo en altura y prevención de caídas desde distintos niveles.',
...
const createEppId = (totalItems: number) => `EPP-${String(totalItems + 1).padStart(3, '0')}`
...
export default function EppDashboard() {
  const { items, loading: firestoreLoading, error: firestoreError, addEpp, updateEpp, deleteEpp } = useEppFirestore()
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editingItemId, setEditingItemId] = useState<string | null>(null)
  const [formState, setFormState] = useState<EppFormState>(() => initialFormState(0))
  ...
  const totals = useMemo(() => {
    ...
    items.forEach((item) => {
      if (item.multiSize) {
        ...
        const hasLow = item.sizeVariants.some((variant) => variant.stockActual <= variant.stockMinimo)
  ...
  const handleVariantChange = (variantId: string, field: keyof SizeVariantForm, value: string) => {
    setFormState((prev) => ({
      ...prev,
  ...
  const handleRemoveVariant = (variantId: string) => {
    setFormState((prev) => {
      ...
      return {
        ...prev,
        sizeVariants: prev.sizeVariants.filter((variant) => variant.id !== variantId),
      }
    })
  }
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      console.error(error)
      setFormError('No se pudo convertir el archivo seleccionado. Intenta nuevamente.')
    }
  }
  ...
  const handleDelete = async (itemId: string) => {
    if (window.confirm('¿Estás seguro de eliminar este EPP? Esta acción no se puede deshacer.')) {
  ...
  const handleToggleDiscontinued = async (itemId: string) => {
    ...
    if (item) {
      const result = await updateEpp(itemId, { discontinued: !item.discontinued })
      if (!result.success) {
  ...
  const handleExportToExcel = () => {
    ...
    // Descargar archivo
    const fileName = `Inventario_EPP_${new Date().toISOString().split('T')[0]}.xlsx`
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (event) => {
    ...
    try {
      ...
      if (formState.multiSize) {
        const invalidVariant = preparedVariants.find((v) => v.stockCritico > v.stockMinimo)
        if (invalidVariant) {
          setFormError(`Talla "${invalidVariant.label}": el stock crítico no puede ser superior al stock mínimo.`)
          setSaving(false)
          return
        }
      }
      ...
      const eppData = {
        ...
        discontinued: editingItemId ? (items.find((i) => i.id === editingItemId)?.discontinued ?? false) : false,
        multiSize: formState.multiSize,
        sizeVariants: preparedVariants,
        stockActual: formState.multiSize ? undefined : Number(formState.stockActual) || 0,
        stockMinimo: formState.multiSize ? undefined : Number(formState.stockMinimo) || 0,
        stockCritico: formState.multiSize ? undefined : Number(formState.stockCritico) || 0,
      ...
      <section className="rounded-4xl border border-white/70 bg-white/95 p-6 shadow-[0_24px_50px_-35px_rgba(15,23,42,0.45)] backdrop-blur dark:border-dracula-current dark:bg-dracula-bg/95">
        ...
        {items.length === 0 ? (
          <div className="mt-10 flex flex-col items-center justify-center gap-4 rounded-3xl border border-dashed border-celeste-200/70 bg-soft-gray-50/70 p-10 text-center">
            ...
            <div>
              <h4 className="text-lg font-semibold text-slate-700">Aún no has cargado EPP</h4>
              <p className="mt-2 text-sm text-slate-500">
                Registra tu primer equipo de protección para comenzar a controlar entregas, stock y certificaciones.
              </p>
            </div>
            <button
              onClick={() => {
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-4 rounded-3xl border border-dashed border-celeste-200/70 bg-soft-gray-50/70 p-10 text-center">
            ...
            <div>
              <h4 className="text-lg font-semibold text-slate-700">No se encontraron resultados</h4>
              <p className="mt-2 text-sm text-slate-500">
                  ...
                  <div className="flex flex-1 flex-col gap-5">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {item.discontinued ? <CheckCircle2 className="h-4 w-4" /> : <Archive className="h-4 w-4" />}
                        </button>
                        <button
                          type="button"
                    ...
                    <div className="rounded-3xl bg-soft-gray-50/80 p-5 text-sm text-slate-600">
                      {item.multiSize ? (
                          ...
                          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            {item.sizeVariants.map((variant) => {
                              ...
                              return (
                                ...
                                >
                                  <p className="text-sm font-semibold text-slate-700">Talla {variant.label}</p>
                                  <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
                                    <span>Stock: {variant.stockActual}</span>
                                    <span className={status}>Mín: {variant.stockMinimo}</span>
                                  </div>
                                  <p className={`mt-1 text-xs ${status}`}>Crítico: {variant.stockCritico}</p>
                                </div>
                              )
                            })}
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 text-center">
                          <div>
                            ...
                            <p
                              className={`mt-1 text-lg font-semibold ${
                                (item.stockActual ?? 0) <= (item.stockCritico ?? 0)
                                  ? 'text-rose-500'
                          ...
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Mínimo</p>
                            <p className="mt-1 text-lg font-semibold text-slate-700">{item.stockMinimo ?? 0}</p>
                          </div>
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Crítico</p>
                            <p className="mt-1 text-lg font-semibold text-slate-700">{item.stockCritico ?? 0}</p>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {item.model ? (
                      <div className="rounded-2xl border border-celeste-100/70 bg-celeste-50/50 px-4 py-3 text-xs text-slate-600">
                        <p className="font-semibold text-slate-700">Protección principal:</p>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setFormState((prev) => ({
                        ...
                        model: event.target.value ? categoryProtectionMap[event.target.value] ?? '' : '',
                      }))
                    }
                    className="rounded-2xl border border-soft-gray-200/70 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-sm focus:border-celeste-300 focus:outline-none"
                    required
                ...
                <div className="flex flex-col gap-1 text-sm text-slate-600">
                  ...
                  <div className="rounded-2xl border border-soft-gray-200/70 bg-soft-gray-50 px-4 py-2.5 text-sm text-slate-600 shadow-inner">
                    {formState.model || 'Selecciona una categoría para sugerir la protección principal.'}
                  </div>
                </div>
                <label className="flex flex-col gap-1 text-sm text-slate-600">
                  Marca
                ...
                <label className="flex flex-col gap-1 text-sm text-slate-600 md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(event) => setFormState((prev) => ({ ...prev, certifications: event.target.value }))}
                    className="rounded-2xl border border-soft-gray-200/70 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-sm focus:border-celeste-300 focus:outline-none"
              ...
              <div className="rounded-3xl border border-soft-gray-200/70 bg-soft-gray-50/70 p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h4 className="text-sm font-semibold text-slate-700">Gestión de stock</h4>
                    <p className="text-xs text-slate-500">Configura si este EPP maneja tallas o talla única.</p>
                  </div>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setFormState((prev) => ({
                          ...
                          sizeVariants: event.target.checked ? prev.sizeVariants : [createSizeVariant()],
                        }))
                      }
                ...
                {formState.multiSize ? (
                  <div className="mt-4 space-y-4">
                    {formState.sizeVariants.map((variant, index) => (
                      <div key={variant.id} className="rounded-2xl border border-white/70 bg-white/80 px-4 py-4 shadow-sm">
                        <div className="flex items-center justify-between gap-3">
                          <h5 className="text-sm font-semibold text-slate-700">Talla #{index + 1}</h5>
                        ...
                        <div className="mt-3 grid gap-3 sm:grid-cols-4">
                          ...
                          <input
                            ...
                            onChange={(event) => handleVariantChange(variant.id, 'stockActual', event.target.value)}
                          ...
                          <input
                            ...
                            onChange={(event) => handleVariantChange(variant.id, 'stockMinimo', event.target.value)}
                          ...
                          <input
                            ...
                            onChange={(event) => handleVariantChange(variant.id, 'stockCritico', event.target.value)}
                    ...
                    <button
                      ...
                      onClick={() =>
                        setFormState((prev) => ({
                          ...prev,
                          sizeVariants: [...prev.sizeVariants, createSizeVariant()],
                        }))
                      }
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs text-slate-600">
                      ...
                      <input
                        ...
                        onChange={(event) => setFormState((prev) => ({ ...prev, stockActual: event.target.value }))}
                        className="rounded-2xl border border-soft-gray-200/70 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-celeste-300 focus:outline-none"
                    ...
                    <label className="flex flex-col gap-1 text-xs text-slate-600">
                      ...
                      <input
                        ...
                        onChange={(event) => setFormState((prev) => ({ ...prev, stockMinimo: event.target.value }))}
                        className="rounded-2xl border border-soft-gray-200/70 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-celeste-300 focus:outline-none"
                    ...
                    <label className="flex flex-col gap-1 text-xs text-slate-600">
                      ...
                      <input
                        ...
                        onChange={(event) => setFormState((prev) => ({ ...prev, stockCritico: event.target.value }))}
                        className="rounded-2xl border border-soft-gray-200/70 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-celeste-300 focus:outline-none"
                  ...
                  <div className="rounded-2xl border border-dashed border-celeste-200/70 bg-celeste-50/40 px-4 py-5 text-center">
                    {formState.imageBase64 ? (
                      <div className="space-y-3">
                        ...
                        />
                        <p className="text-xs font-medium text-slate-600">{formState.photoName}</p>
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setFormState((prev) => ({ ...prev, imageBase64: undefined, photoName: undefined }))
                            }
                      ...
                      <>
                        ...
                        <p className="mt-2 text-xs text-slate-500">Formatos PNG o JPG. Se almacenará en base64.</p>
                      </>
                    )}
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(event) => handleFileUpload(event, 'imageBase64', 'photoName')}
                      className="hidden"
                      id="epp-photo-upload"
                    />
                  </div>
                  ...
                  <div className="rounded-2xl border border-dashed border-celeste-200/70 bg-celeste-50/40 px-4 py-5 text-center">
                    {formState.technicalSheetBase64 ? (
                      <div className="space-y-3">
                        ...
                        <p className="text-xs font-medium text-slate-600">{formState.technicalSheetName}</p>
                        <div className="flex items-center justify-center gap-2">
                          <label
                      ...
                      <>
                        ...
                        <p className="mt-2 text-xs text-slate-500">Máximo 5 MB. Se convierte a base64.</p>
                      </>
                    )}
                    <input
                      type="file"
                      accept="application/pdf"
                      onChange={(event) => handleFileUpload(event, 'technicalSheetBase64', 'technicalSheetName')}
                      className="hidden"
                      id="epp-tech-upload"
                    />
                  </div>
                  ...
                  <div className="rounded-2xl border border-dashed border-celeste-200/70 bg-celeste-50/40 px-4 py-5 text-center">
                    {formState.certificatesBase64 ? (
                      <div className="space-y-3">
                        ...
                        <p className="text-xs font-medium text-slate-600">{formState.certificatesName}</p>
                        <div className="flex items-center justify-center gap-2">
                          <label
                      ...
                      <>
                        ...
                        <p className="mt-2 text-xs text-slate-500">Puedes subir un archivo con todas las certificaciones.</p>
                      </>
                    )}
                    <input
                      type="file"
                      accept="application/pdf"
                      onChange={(event) => handleFileUpload(event, 'certificatesBase64', 'certificatesName')}
                      className="hidden"
                      id="epp-cert-upload"
                    />
                  </div>
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                ...
                >
                  {saving ? 'Guardando...' : editingItemId ? 'Actualizar EPP' : 'Guardar EPP'}
                </button>
              </div>
            </form>
          </div>
